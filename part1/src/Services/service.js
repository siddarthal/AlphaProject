...
const editAccountDetails = (formData,token1) => {
  const header1 = {
    Authorization: "Bearer " + token1,
  };
  // const body = {
  //   name: name,
  // };
  return axios.put(urserAccountUrl, formData, { headers: header1 });
};
...
const getTicketsBooked = (id, token) => {
  const url = backendURL + `/api/tickets/event/${id}/`;
  const header1 = {
    Authorization: "Bearer " + token,
  };
  return axios.get(url, { headers: header1 });
